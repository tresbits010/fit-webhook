...
async function processLicensePaymentById(paymentId) {
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      // Leemos el estado actual de la licencia para calcular los días
      ...
      // =========================================================
      // 🔥 LÓGICA ANTI-TRAMPA DE FECHAS (Periodos de Gracia) 🔥
      // =========================================================
      let fechaInicio = new Date(); // Por defecto arranca hoy

      if (prev.expiryUtc) {
        const prevExpiry = new Date(prev.expiryUtc);
        const diasVencido = (fechaInicio - prevExpiry) / (1000 * 60 * 60 * 24);

        // CASO 1: Pago ADELANTADO (Todavía no se le venció)
        // -> Sumamos los nuevos días al final de su vencimiento futuro.
        if (diasVencido < 0) {
            fechaInicio = prevExpiry;
        } 
        // CASO 2: Pago ATRASADO pero dentro de gracia/tolerancia (ej: menos de 7 días)
        // -> El nuevo mes arranca a contar retroactivamente desde que se le venció.
        else if (diasVencido >= 0 && diasVencido <= 7) {
            fechaInicio = prevExpiry;
        }
        // CASO 3: Si pasaron más de 7 días, arranca desde hoy (Borrón y cuenta nueva).
      }

      const expiry = new Date(fechaInicio); 
